...
public class Application3 {
    public static void main(String[] args) {
        ...
        * 또한, 규칙에 맞는 올바른 변수명을 짓는 것이 좋은 프로그래밍을 하는 첫 단주이다.
        * */
        ...
        *   1-4 변수명은 숫자로 시작할 수 없다. (시작만 할 수 없고 숫자를 섞는 건 가능하다.)
        *   1-5 특수 기호는 '_'와 '$'만 사용 가능하다. (공백을 사용할 수 없습니다.)
        * 2. 컴파일 에러를 발생시키지는 않지만 개발자들끼리의 규칙(암무적 규칙)
        *   2-1 변수명의 길이 제한은 없다. (적당히)
        *   2-1 변수명이 합성어로 이루어진 경우, 첫 단어는 소문자, 두번째 시작단어부터는 대문자로 시작한다.(camel-case)
        *   2-3 단어와 단어 사이의 연결은 언더스코어(_)로 하지 않는다. (2-2)
        *       상수를 사용할 때 특수하게 사용한다.
        ...
        * */
        ...
        //int a ge; // 사용 가능한 특수 문자 외에는 사용이 불가능하다.
        //int a@ge;
        int _age = 20; // 언더스코어(_) 사용 가능하다. 처음 중간 마지막 상관 없음.
        int sh$arp$ = 20; // $ 사용 가능하다. 처음 중간 마지막 상관 없음.


        // 암묵적 약속

        // 변수의 길이에는 제한이 없지만, 적당히 하자.
        int shdkajhdjkdasdawdgfddsgwwgfd; // 이렇게는 사용하지 말자.

        /*
        * 카멜케이스
        *
        * 합성어일 경우 첫 단어는 모두 소문자, 두번째 시작 단어는 대문자로 시작한다.
        * 자바에서는 클래스명만 유일하게 대문자로 시작된다.
        * */
        int maxAge = 20;
        int minAge = 10;
