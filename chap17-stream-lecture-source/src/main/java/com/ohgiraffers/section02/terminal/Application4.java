...
public class Application4 {
    public static void main(String[] args) {
        ...
        * boolean nonMatch(predicate<? super T> predicate); // 모든 조건을 만족하지 않는지
        * */

        List<String> stringList = Arrays.asList("jaava", "Spring", "SpringBoot");

        // 하나라도 조건을 만족하는 값이 있는지 확인
        boolean anMatch = stringList.stream()
                .anyMatch(str -> str.contains("p")); // 문자열이 "p"를 포함하고 있는지
        ...
        // 모든 조건을 만족하지 않는지
        boolean nonMatch = stringList.stream()
                .noneMatch(str -> str.contains("c")); // 문자열이 "c"를 포함하지 않는지

        System.out.println("nonMatch = " + nonMatch); // true
    }
}
